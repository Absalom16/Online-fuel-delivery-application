...
<head>
  ...
  <title> Available diesel orders </title>
...
<div class="col-sm-8">
<h2 class="text-center" style="color:black">These are the diesel orders made</h2>
<p>
<?php
try {
 ...
 $q = "SELECT COUNT(order_id) FROM diesel_orders";
 ...
 $query .=
 " AS regdat, order_id FROM diesel_orders ORDER BY order_date ASC";
 $query .=" LIMIT ?, ?";
...
$order_date = htmlspecialchars($row['regdat'], ENT_QUOTES);
 ...
 <td><a href="delete-diesel-orders.php?id=' . $order_id .
 '">Delete</a></td>
 <td>' . $first_name . '</td>
<td>' . $last_name . '</td>
<td>' . $email . '</td>
...
<td>' . $order_date . '</td>
 ...
 if ($current_page != 1) {
 $echostring .=
 '<a href="admin-view-diesel-orders.php?s=' . ($start - $pagerows) .
 '&p=' . $pages . '">Previous</a> ';
 }
