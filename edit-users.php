        ...
        <!--shell-->
          <div class="col-sm-4 col-lg-12 all shell">
            <div class="box">
              <div>
                <div class="img-box">
                  <img src="images/petrol station.jpg" alt="">
                </div>
...
$errors[] = ' or the e-mail format is incorrect.';
 ...
 if (empty($user_level)) {
 $errors[] = 'You forgot to enter the user level.';
 }
 ...
 $query = 'UPDATE users SET first_name=?, last_name=?, email=?, phone_number=?, user_level=?';
 $query .= ' WHERE user_id=? LIMIT 1';
$q = mysqli_stmt_init($dbcon);
mysqli_stmt_prepare($q, $query);
// bind values to SQL Statement
mysqli_stmt_bind_param($q, 'sssssi', $first_name, $last_name, $email, $phone_number, $user_level, $id);
 // execute query
mysqli_stmt_execute($q);
if (mysqli_stmt_affected_rows($q) == 1) { // Update OK
 ...
 $q = mysqli_stmt_init($dbcon);
 $query = "SELECT first_name, last_name, email, phone_number, user_level FROM users WHERE user_id=?";
 mysqli_stmt_prepare($q, $query);
 // bind $id to SQL Statement
...
<div class="form-group row">
 <label for="user_level" class="col-sm-4 col-form-label">
 User level:</label>
<div class="col-sm-8">
 <input type="text" class="form-control" id="user_level" name="user_level"
 placeholder="User level" maxlength="40" required
