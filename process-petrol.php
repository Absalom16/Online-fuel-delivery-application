...
try{
    ...
    if(empty($errors)){//if everything is ok, record to database
        ...
        //make the query
        $query="INSERT INTO petrol_orders (order_id, price, tax, delivery_fee, quantity, total,company, gas_station, location, phone_number, amount_paid, order_date)";
        $query .="VALUES(' ',?,?,?,?,?,?,?,?,?,?, NOW() )";
    ...
    if(mysqli_stmt_affected_rows($q)==1) {//one reord inserted
        header ("location:petrol-db-update-form.php");
        exit();
